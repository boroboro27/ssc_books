...
def subscribe_book(book_id):
    ...
    if 'userLogged' in session: 
        ...
        if not res[0]:
            ...
            flash((f"Подписка #{res[1]} успешно оформлена. Теперь мы будем сообщать вам, если книга возвращается на полку "                           
...
def lk():
    if 'userLogged' in session:
        ...
        return render_template('lk.html', title='Личный кабинет',
                                   ...
                                   subscriptions=dbase.getSubscriptions(user_id[0]), 
...
def close_feedback(fb_id):
    ...
    if 'userLogged' in session: 
        ...
        if not res[0]:
            flash(f"Ошибка при закрытии обращения #{fb_id}: {res[1]}.", category='error')
        else:
            flash((f'Обращение #{fb_id} закрыто (закрыто обращений в БД: {res[1]})'), category='success')        
